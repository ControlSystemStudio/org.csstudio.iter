...
public class DBContentProvider implements IAutoCompleteProvider {
    ...
    public DBContentProvider() {
        ...
        try {
            ...
            Activator.getLogger().log(Level.SEVERE, "Failed to generate DBD config template: " + e.getMessage());
        }
    }

    @Override
    public boolean accept(final ContentType type) {
        if (type == ContentType.PVName || type == ContentType.PVField || type == ContentType.PVParam)
            return true;
        return false;
    }
    ...
    public AutoCompleteResult listResult(final ContentDescriptor desc, final int limit) {
        ...
        if (pvDesc.getContentType() == ContentType.PVField && pvDesc.getName() != null && !pvDesc.getName().isEmpty()
                ...
                for (Field f : fieldMap.get(type)) {
                    if (f.getName().startsWith(pvDesc.getField())) {
                        if (count <= limit) {
                            ...
                            proposal.setInsertionPos(pvDesc.getStartIndex());
                            result.addProposal(proposal);
                            if (closestMatchingField == null || closestMatchingField.compareTo(f.getName()) > 0) {
