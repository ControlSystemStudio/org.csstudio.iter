...
public class OlogAuthAdapter implements SecurityListener {
    ...
    private static final Logger log = Logger.getLogger(OlogAuthAdapter.class.getName());
    ...
    public void changedSecurity(Subject subject, boolean is_current_user, Authorizations authorizations) {
        ...
        prefStore.setValue(org.csstudio.utility.olog.PreferenceConstants.Use_authentication, true);
        String username;
        String password;
        String connectedOlogUsername = prefs.getString(Activator.PLUGIN_ID, PreferenceConstants.connected_olog_user,
                null, null);
        ...
        if (anonymousLogin) {
            ...
            username = prefs.getString(Activator.PLUGIN_ID, PreferenceConstants.anonymous_olog_user, null, null);
            password = prefs.getString(Activator.PLUGIN_ID, PreferenceConstants.anonymous_olog_password, null, null);
            if (username != null) {
                prefStore.setValue(org.csstudio.utility.olog.PreferenceConstants.Username, username);
                prefStore.setValue(org.csstudio.utility.olog.PreferenceConstants.Password, password);
            }
        } else {
            // Set default olog user to tagmod
            username = connectedOlogUsername;
            password = prefs.getString(Activator.PLUGIN_ID, PreferenceConstants.connected_olog_password, null, null);
            if (username != null) {
                prefStore.setValue(org.csstudio.utility.olog.PreferenceConstants.Username, username);
                prefStore.setValue(org.csstudio.utility.olog.PreferenceConstants.Password, password);
            }
        ...
        if (username != null) {
            ...
            String url = prefs.getString(org.csstudio.utility.olog.Activator.PLUGIN_ID,
                    org.csstudio.utility.olog.PreferenceConstants.Olog_URL, "http://localhost:8080/Olog/resources",
                    null);
            ...
            log.log(Level.INFO, "Creating Olog client : " + url + " with user '" + username + "'");
            try {
