...
public class DbdParser {
    ...
    private void visitTree(final Tree _tree) throws DbdParsingException {
        ...
        switch (type) {
            case DbdFileParser.PATH:
                this.currentTemplate.getPaths().add(new Path(getText(_tree)));
                break;
            case DbdFileParser.INCLUDE:
                this.currentTemplate.getIncludes().add(new Include(getText(_tree)));
                break;
            ...
            case DbdFileParser.DRIVER:
                this.currentTemplate.getDrivers().add(new Driver(getText(_tree)));
                break;
            case DbdFileParser.REGISTRAR:
                this.currentTemplate.getRegistrars().add(new Registrar(getText(_tree)));
                break;
            case DbdFileParser.VARIABLE:
                this.currentTemplate.getVariables().add(new Variable(getText(_tree)));
                break;
            case DbdFileParser.FUNCTION:
                this.currentTemplate.getFunctions().add(new Function(getText(_tree)));
                break;
            case DbdFileParser.BREAKTABLE:
                visitBreaktable(_tree);
                break;
            default:
                throw new DbdParsingException("Unknown tree type: " + _tree.getText());
        }
    }
            ...
            if (menuChild != null) {
                ...
                switch (type) {
                    ...
                    case DbdFileParser.INCLUDE:
                        ...
                        break;
                    default:
                        throw new DbdParsingException("Unknown MENU tree type: " + menuChild.getText());
                }
            }
        }
        if (menu != null)
        ...
        for (int i = 0; i < childrenNum; i++) {
            ...
            if (recordTypeChild != null) {
                ...
                switch (type) {
                    ...
                    case DbdFileParser.INCLUDE:
                        if (recordType != null)
                            recordType.addInclude(new Include(getText(recordTypeChild)));
                        break;
                    default:
                        throw new DbdParsingException("Unknown RECORDTYPE tree type: " + recordTypeChild.getText());
                }
            }
        }
        if (recordType != null)
    ...
    private void visitField(final Tree _tree, RecordType recordType) throws DbdParsingException {
            ...
            if (fieldChild != null) {
                ...
                switch (type) {
                    ...
                    case DbdFileParser.RULE:
                        ...
                        break;
                    default:
                        throw new DbdParsingException("Unknown FIELD tree type: " + fieldChild.getText());
                }
            }
        }
    ...
    private void visitDevice(final Tree _tree) throws DbdParsingException {
        final int childrenNum = _tree.getChildCount();
        if (childrenNum < 4)
            throw new DbdParsingException("Missing text child in DEVICE node: " + childrenNum);
        ...
        this.currentTemplate.getDevices().add(new Device(recordType, linkType, dsetName, choiceString));
    }
            ...
            if (btChild != null) {
                ...
                switch (type) {
                    ...
                    case DbdFileParser.VALUE:
                        ...
                        break;
                    default:
                        throw new DbdParsingException("Unknown BREAKTABLE tree type: " + btChild.getText());
                }
            }
        }
        if (bt != null)
    ...
    private String getText(final Tree _tree) throws DbdParsingException {
        if (_tree.getChild(0) == null)
            throw new DbdParsingException("Missing text child in node: " + _tree.getText());
