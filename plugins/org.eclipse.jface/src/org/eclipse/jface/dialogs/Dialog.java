 ...
 * </p>
 * <p>
 * If more than one modal dialog is open, the second one should be parented off
 * of the shell of the first one. Otherwise, it is possible that the OS will
 * give focus to the first dialog, potentially blocking the UI.
 * </p>
 * <p>
...
public abstract class Dialog extends Window {
    ...
    /**
     * Image registry key for info image (value
     * <code>"dialog_info_image"</code>).
     *
     * @deprecated use
     ...
     * if necessary. If a <code>null</code> value is given, <code>null</code> is
     * returned.
     *
     * @param textValue
    ...
    //added as fix to ITER CSS popup window -- START

    /**
	 * Constructs a new dummy shell which can be used as a parent to dialogs
	 * that are to be shown on top of the full screen workbench window. This
	 * shell needs to be disposed of when no longer needed.
	 * 
	 * @param parentShell
	 *            the parrent Shell
	 *
    ...
    public static Shell createDummyShell(Shell parentShell) {
        if (parentShell != null) {
            ...
            if (!fullScreen[0]) {
                // if the parent is not in full screen, just use that as a
                // parent
                return parentShell;
            }
        }
        ...
        //added as fix to ITER CSS popup window -- END
     ...
     * dialog's return code to <code>Window.CANCEL</code> and closes the dialog.
     ...
     * <b>Cancel</b>. On some platforms, {@link #initializeBounds()} will move
     * the default button to the right.
     * </p>
     *
     * @param parent
     ...
     * required by the {@link Display#getDismissalAlignment() platform
     * convention}.
     * </p>
     */
    @Override
    ...
    /**
     * Returns the button created when <code>createButton</code> is called with
     * an ID of <code>IDialogConstants.CANCEL_ID</code>. If
     ...
     * </p>
     *
     * @return the dialog area, or <code>null</code> if the dialog area has not
     *         been created yet
     */
     ...
     *         backwards compatibility they are still supported, however new
     *         code should use SWT for these.
     *
    ...
    /**
     * Returns the button created when <code>createButton</code> is called with
     * an ID of <code>IDialogConstants.OK_ID</code>. If
     * <code>createButton</code> was never called with this parameter, or if
     * <code>createButton</code> is overridden, <code>getOKButton</code> will
     ...
     * of of the dialog, according to the dialog bounds strategy.
     *
    ...
    /**
     * Returns the initial size to use for the shell. Overridden to check
     * whether a size has been stored in dialog settings. If a size has been
     * stored, it is returned.
     *
     ...
     * whether the bounds of the dialog have been stored in dialog settings. If
     * a location has been stored, it is returned.
     *
     * @param initialSize
     ...
     *
     * This method is used to ensure that all style bits appropriate for
     * resizable dialogs are added to the shell style. Individual dialogs may
     * always set the shell style to ensure that a dialog is resizable, but
     * using this method ensures that resizable dialogs will be created with the
     * same set of style bits.
     *
     * Style bits will never be removed based on the return value of this
     * method. For example, if a dialog returns <code>false</code>, but also
     * sets a style bit for a SWT.RESIZE border, the style bit will be honored.
     *
