...
public class SDDPVListProvider implements IAutoCompleteProvider {
    ...
    public SDDPVListProvider() {
        try {
            rdb = RDBUtil.connect(Preferences.getRDB_Url(), Preferences.getRDB_User(), Preferences.getRDB_Password(),
                    true);
            Activator.getLogger().log(Level.INFO, "SDDPVListProvider connected to DB: " + Preferences.getRDB_Url());
        } catch (Exception e) {
    ...
    public AutoCompleteResult listResult(final ContentDescriptor desc, final int limit) {
        ...
        try {
            ...
            if ("!ERROR: canceling statement due to user request".equals(e.getMessage())) {
                Activator.getLogger().log(Level.WARNING, e.getMessage());
            }
        } finally {
            try {
