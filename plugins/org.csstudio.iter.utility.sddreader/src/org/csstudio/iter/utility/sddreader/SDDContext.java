...
public class SDDContext implements Serializable {
    ...
    private final String pv_get_fields = "SELECT at.fieldname, fa.attvalue FROM functionalvariables as fv "
            ...
            + "INNER JOIN variableattributetypes as at ON at.id = fa.atttype_id " + "WHERE fv.id = ?";
    ...
    public SDDContext() {
        try {
            rdb = RDBUtil.connect(Preferences.getRDB_Url(), Preferences.getRDB_User(), Preferences.getRDB_Password(),
                    true);
            Activator.getLogger().log(Level.INFO, "SDDContext connected to DB: " + Preferences.getRDB_Url());
        } catch (Exception e) {
    ...
    public List<Record> findRecord(String name) {
        ...
        try {
            ...
            try {
                if (statement_fields != null)
                    statement_fields.close();
                if (statement_pv != null)
                    statement_pv.close();
