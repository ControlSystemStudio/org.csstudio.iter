...
function displayErrorMsg() {
	...
	while (i < pvs.length) {
		...
		if (valid != OK && valid != UNCHANGED) {
			switch(valid) {
				case BADINPUT:
					errorMsg.push("Alarm Setpoint for " + pvs[i+1].getName() + " is invalid and cannot be converted to a value" + "\n");
					break;
				case RANGEOVER:
					errorMsg.push("Alarm Setpoint for " + pvs[i+1].getName() + " is invalid as greater than the trip setpoint" + "\n");
					break;
				default:
